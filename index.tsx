...
const App = () => {
  const [bundles, setBundles] = useState<Bundle[]>(DataService.loadBundles);
  const [allergens, setAllergens] = useState<{ code: string, name: string }[]>(DataService.loadAllergens);
  ...
  useEffect(() => { DataService.saveBundles(bundles); }, [bundles]);
  useEffect(() => { DataService.saveAllergens(allergens); }, [allergens]);
  ...
  const generatePDF = async (shouldDownload: boolean = false) => {
    ...
    const labelRoot = createRoot(renderContainer);
    
    for (let i = 0; i < flatLabels.length; i++) {
      const bundle = flatLabels[i];
      
      labelRoot.render(<LabelPreview bundle={bundle} date={packedOn} t={t} lang={lang} allergens={allergens} />);
      ...
      <nav className="brand-green text-white px-8 py-5 flex flex-col md:flex-row items-center justify-between shadow-2xl sticky top-0 z-50">
        <div className="flex items-center gap-4 mb-4 md:mb-0">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-60">Label Engine v2.3</p>
          </div>
        </div>
                  ...
                  <div className="space-y-8">
                    <div className="bg-slate-800/30 p-8 rounded-[2rem] border border-slate-800">
                      ...
                      <div className="mt-10 flex flex-col sm:flex-row gap-5">
                        <input type="file" id="csv-upload" className="hidden" accept=".csv, .xlsx, .xls" onChange={handleFileUpload} />
