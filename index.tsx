...
import { getFirestore, collection, getDocs, doc, deleteDoc, writeBatch, setDoc } from 'firebase/firestore';
...
const Label: React.FC<{ bundle: Bundle, lang: 'de' | 'en', packedOn: string, forPrint?: boolean }> = ({ bundle, lang, packedOn, forPrint }) => {
  const getDietIcon = (diet: string) => {
    ...
    if (d.includes('vegan') || d.includes('vegetarisch')) return <Leaf size={24} className="text-[#024930]" />;
    if (d.includes('meat') || d.includes('beef') || d.includes('parma')) return <Beef size={24} className="text-red-700" />;
    return <Soup size={24} className="text-blue-500" />;
  };

  return (
    <div 
      className={`label-card w-[100mm] h-[62mm] bg-white text-slate-900 flex flex-col overflow-hidden relative ${!forPrint ? 'shadow-2xl border border-slate-700 rounded-lg scale-90 sm:scale-100 origin-top' : 'page-break-after-always'}`} 
      style={{ 
        color: '#000', 
    ...
    >
      ...
      <div className="bg-[#024930] py-3 px-6 flex items-center justify-center min-h-[50px]">
        <h2 className="text-white text-center font-black text-xl uppercase tracking-wider leading-tight">
          {lang === 'de' ? bundle.name_de : bundle.name_en}
        </h2>
      </div>
      
      {/* Brand Stripe */}
      <div className="w-full h-[3px] bg-[#FEACCF]"></div>

      {/* Content Body */}
      <div className="flex-1 px-5 py-4 flex flex-col overflow-hidden">
        <div className="space-y-3">
          {bundle.items.map(item => (
            <div key={item.id} className="flex justify-between items-start border-b border-dotted border-gray-100 pb-2 last:border-none">
              <div className="flex-1 pr-4">
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {item.allergens_de.split(/[,/]+/).map((alg, idx) => {
                    ...
                    return (
                      <span key={idx} className="bg-[#FEACCF] text-[9px] font-black px-2 py-0.5 rounded-[4px] uppercase text-[#024930] tracking-tighter">
                        {trimmed}
                      </span>
                    );
                  })}
              ...
              <div className="flex flex-col items-center min-w-[75px] text-center">
                <div className="p-1 rounded-full bg-slate-50 border border-slate-100 shadow-sm mb-1">
                  {getDietIcon(item.diet_de)}
                </div>
                <span className="text-[10px] font-black uppercase tracking-tight text-[#024930]">
                  {item.diet_de}
                </span>
              </div>
            </div>
      ...
      {/* Footer Bar */}
      <div className="bg-[#C197AB] py-3 px-6 flex justify-between items-center text-[#024930] mt-auto">
         ...
         <div className="font-black text-xl tracking-tighter italic">BELLA&BONA</div>
      </div>
    </div>
  );
};
...
const App: React.FC = () => {
  ...
  return (
    <>
      {/* PRINT AREA: Absolutely top-level, only visible during print */}
      <div id="print-area" className="print-only">
        {selections.map(sel => {
          ...
          for (let i = 0; i < sel.quantity; i++) {
            copies.push(
              <Label key={`${sel.bundleId}-${i}`} bundle={bundle} lang={lang} packedOn={packedOn} forPrint />
            );
          }
          return copies;
        })}
          ...
          <aside className="w-20 lg:w-64 bg-slate-900 border-r border-slate-800 flex flex-col transition-all flex-shrink-0">
            ...
            <nav className="flex-1 p-4 space-y-2">
              <button 
                ...
                className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all ${activeTab === 'generator' ? 'bg-[#FEACCF] text-[#024930] shadow-lg shadow-pink-500/10' : 'text-slate-400 hover:bg-slate-800'}`}
              >
                <Printer size={20} />
              ...
              <button 
                ...
                className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all ${activeTab === 'database' ? 'bg-[#FEACCF] text-[#024930] shadow-lg shadow-pink-500/10' : 'text-slate-400 hover:bg-slate-800'}`}
              >
                <Database size={20} />
            ...
            <div className="p-4 border-t border-slate-800 space-y-4">
              <div className="hidden lg:block px-2">
                ...
                <input 
                  ...
                  className="w-full bg-slate-800 border-none rounded-lg px-3 py-2 text-xs font-bold text-white focus:ring-1 focus:ring-pink-400" 
                />
              </div>
                ...
                <div className="xl:col-span-5 space-y-8 animate-fade-in">
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-900 border-none rounded-2xl pl-14 pr-6 py-4 text-sm font-medium text-white focus:ring-2 focus:ring-emerald-500 transition-all shadow-xl" 
                    />
                  </div>
                  
                  <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-3 custom-scrollbar">
                    <div className="flex items-center justify-between px-2">
                      <h3 className="text-xs font-black text-slate-600 uppercase tracking-widest">{t.availableBundles}</h3>
                      <button onClick={init} className="text-[10px] font-black text-emerald-500 uppercase flex items-center gap-1 hover:bg-slate-800 px-2 py-1 rounded">
                        <RefreshCw size={10} className={isSyncing ? 'animate-spin' : ''} /> Refetch
                      </button>
                    </div>
                    {filteredBundles.length === 0 ? (
                      <div className="text-slate-700 font-bold italic p-10 text-center">No bundles found.</div>
                    ) : (
                ...
                <div className="xl:col-span-7">
                  <section className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl flex flex-col h-full min-h-[500px]">
                    <div className="flex items-center justify-between mb-10">
                      ...
                      {selections.length > 0 && (
                        <button onClick={() => setSelections([])} className="text-xs font-black text-red-400 uppercase tracking-widest px-4 py-2 rounded-xl hover:bg-red-500/10">
                          {t.clearAll}
                        </button>
                      )}
                    </div>
                    ...
                    {selections.length > 0 && (
                      <div className="mt-12 pt-10 border-t border-slate-800 flex items-center justify-between">
                         <button onClick={() => setIsPreviewing(true)} className="flex items-center gap-3 text-emerald-400 font-black text-sm uppercase tracking-widest hover:text-emerald-300">
                           <Eye size={20} /> {t.previewPdf}
                         </button>
                         <button onClick={handlePrint} className="bg-emerald-500 text-slate-950 font-black px-12 py-5 rounded-2xl shadow-xl flex items-center gap-4 text-lg hover:bg-emerald-400 transition-colors">
                ...
                <section className="bg-slate-900 rounded-[3rem] p-12 shadow-2xl border border-slate-800">
                  <div className="flex items-center justify-between mb-12">
                    <div>
                      ...
                      <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Bundle Management</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-14">
                    <div onClick={() => {
                      ...
                      XLSX.writeFile(wb, "BellaBona_Template.xlsx");
                       ...
                       <p className="text-slate-500 text-sm mt-2">Get the standard Excel format</p>
                    </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xs font-black text-slate-600 uppercase tracking-widest px-4">{t.availableBundles}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {bundles.length === 0 ? (
                        <div className="col-span-2 text-center py-10 text-slate-700 font-bold italic">No bundles available.</div>
                      ) : (
                        bundles.map(bundle => (
                          <div key={bundle.id} className="flex items-center justify-between p-6 bg-slate-950/50 rounded-2xl border border-slate-800 shadow-sm">
                            <div className="flex items-center gap-5">
                               ...
                               <div>
                                  ...
                                  <p className="text-[10px] font-bold text-emerald-500 uppercase">{bundle.items.length} items</p>
                               </div>
                            </div>
                            <button 
                              onClick={async () => {
                              ...
                              className="p-3 text-slate-700 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-colors"
                            >
                              <Trash2 size={20} />
                            </button>
                          </div>
      ...
      {isPreviewing && (
        <div className="no-print fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4 sm:p-8 animate-fade-in">
          <div className="bg-slate-900 w-full max-w-5xl h-[90vh] rounded-[2rem] sm:rounded-[3rem] shadow-2xl flex flex-col border border-slate-800 overflow-hidden">
            <div className="p-6 sm:p-10 border-b border-slate-800 flex items-center justify-between">
              <div>
                <h2 className="text-2xl sm:text-3xl font-black text-white">{t.previewPdf}</h2>
                <p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mt-1">Check layout before printing</p>
              </div>
              <button onClick={() => setIsPreviewing(false)} className="w-12 h-12 rounded-2xl bg-slate-800 flex items-center justify-center text-white hover:bg-slate-700 transition-colors">
                <X size={24} />
              </button>
            </div>
            
            <div className="flex-1 overflow-y-auto p-4 sm:p-12 bg-slate-950 flex flex-wrap justify-center gap-8 custom-scrollbar">
              {selections.map(sel => {
                const bundle = bundles.find(b => b.id === sel.bundleId);
                if (!bundle) return null;
                const copies = [];
                for (let i = 0; i < sel.quantity; i++) {
                  copies.push(<Label key={`${sel.bundleId}-${i}`} bundle={bundle} lang={lang} packedOn={packedOn} />);
                }
                return copies;
              })}
            </div>
            ...
            <div className="p-6 sm:p-10 border-t border-slate-800 flex gap-6">
              <button 
                onClick={handlePrint} 
                className="flex-1 bg-emerald-500 text-slate-950 font-black py-4 sm:py-6 rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl flex items-center justify-center gap-4 text-lg sm:text-xl hover:bg-emerald-400 transition-colors"
              >
                <Printer size={28} /> {t.generatePdf}
              </button>
            </div>
